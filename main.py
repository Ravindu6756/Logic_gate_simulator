...
class LogicSandbox:
    ...
    def save(self, name):
        with open('data/saves.txt', 'r', encoding='utf-8')as f:
            for num, line in enumerate(f, start=1):
                if (num-1) % 3 == 0:
                    if name == line:
                        print("Chip is already own this name!")
                        return
        with open('data/saves.txt', 'a') as f:
            f.write(f"{name}\n")
            names = self.memory.keys()
            for i in names:  
                f.write(f"{i},{type(self.memory[i]).__name__};")
            f.write("\n")
            a, b = None, None
            for i,j,k in self.wires:
                for name1, value in self.memory.items():
                    if value == i:
                        a = name1
                    if value == j:
                        b = name1
            ...
            f.write("\n")  
            print("Chip saved!")
    def load(self, memory_save, wire_save):
        self.memory = {}
        self.wires = []
        for name, comp_type in memory_save:
            if comp_type == "InputComponent":
                comp_type = "IN"
            elif comp_type == "OutputComponent":
                comp_type = "OUT"
            elif comp_type == "ProcessComponent":
                comp_type = "NAND"
            self.add(name,comp_type)
        for x, y, node in wire_save:
            self.connect(x,y,node)
...
def main():
    ...
    while True:
        ...
        if bc == "ADD" and len(parts) == 3:
            ...
            select = input("select to load: ")
            if select not in list1:
                print("invalid selection!")
                continue
            memory_save, wire_save = None, None
            with open('data/saves.txt', 'r', encoding='utf-8')as f:
                for num, line in enumerate(f, start=1):
                    if (num-1) % 3 == 0:
                        memory_save = f.readline()
                        wire_save = f.readline()
            #break string into list
            m0 = memory_save.strip().split(";")
            m1 = wire_save.strip().split(";")
